...
module Insales # :nodoc:
  ...
  class YandexMetrika
    ...
    def self.yandex_metrika_code(options={})
      if options[:new_ya_metrika_analytics]
        new_counter(options[:product_detail_options] || options)
      else
        old_counter(options[:order] || options)
      end
    end

    def self.old_counter(options)
      code = <<-HTML
<!-- Yandex.Metrika counter -->
<script type="text/javascript">
  var yaParams = #{options.to_json};
</script>
...
(function (d, w, c) {
    (w[c] = w[c] || []).push(function() {
        try {
            w.yaCounter#{tracker_id} = new Ya.Metrika({id:#{tracker_id},
                    webvisor:true,
                    clickmap:true,
                    ecommerce:true,
